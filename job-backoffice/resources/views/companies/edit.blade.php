<x-app-layout>
    ...
    <div class="py-10 px-4 bg-gray-50 min-h-screen">
        <div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg">
            ...
            <form action="{{ route('companies.update', ['company'=>$company->id,'redirectToList'=>request('redirectToList')]) }}" method="POST" class="space-y-10">
                @csrf
                @method('put')
                {{-- Company Details --}}
                <div class="space-y-5">
                    <h3 class="text-lg font-bold text-gray-800 border-b pb-2">🏢 Company Details</h3>
                    <p class="text-sm text-gray-500">the company details below</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" placeholder="Enter company name"
                            value="{{ old('name', $company->name) }}" class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none transition
                    ...
                    <div>
                        ...
                        <input type="text" name="address" id="address" placeholder="Enter company address"
                            value="{{ old('address', $company->address) }}" class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none transition
                    ...
                    <div>
                        ...
                        <input type="text" name="website" id="website" placeholder="Enter company website"
                            value="{{ old('website', $company->website) }}" class="w-full px-4 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none transition
                ...
                <div class="space-y-5">
                    ...
                    {{-- Owner Name --}}
                    <div>
                        <label for="owner_name" class="block text-sm font-medium text-gray-700 mb-1">👨‍💼 Owner
                            Name</label>
                    ...
                    <div>
                        <label for="owner_email" class="block text-sm font-medium text-gray-700 mb-1">📧 Owner
                            Email</label>
                        <input type="email" name="owner_email" id="owner_email" placeholder="Enter owner email "
                            value="{{ old('owner_email', $owner->email) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none bg-gray-100"
                            disabled>
                        @error('owner_email')
                            <p class="mt-1 text-sm text-red-600">⚠️ {{ $message }}</p>
                        @enderror
                    ...
                    <div x-data="{ showPassword: false }">
                        <label for="owner_password" class="block text-sm font-medium text-gray-700 mb-1">🔑 Owner
                            Password (Leave blank to keep the same)</label>
                        <div class="relative">
